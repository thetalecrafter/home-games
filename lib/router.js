/*global window, document, location, history*/
import pathToRegexp from 'path-to-regexp'

export default function Router ({ useHash=false }={}) {
  if (!(this instanceof Router)) {
    return new Router({ useHash })
  }

  this.useHash = useHash === true ? '#' : useHash
  this.routes = []
  this.routeFromLocation = this.routeFromLocation.bind(this)
  this.routeFromLinkClick = this.routeFromLinkClick.bind(this)
}
Router.prototype = {
  start ({ routeLinks=false }) {
    const evtname = this.useHash ? 'hashchange' : 'popstate'
    window.addEventListener(evtname, this.routeFromLocation, false)
    if (routeLinks) {
      document.addEventListener('click', this.routeFromLinkClick, false)
    }
  ...
  stop () {
    ...
    document.removeEventListener('click', this.routeFromLinkClick, false)
  },
  ...
  routeFromLinkClick (event) {
    if (event.metaKey || event.shiftKey || event.ctrlKey || event.altKey) {
      return // ignore if it could open a new window
    }
    const html = document.documentElement
    let target = event.target
    while (!target.href && target !== html) { target = target.parentNode }
    if (!target.href) { return } // not a link click

    const origin = location.origin || (location.protocol + '//' + location.host)
    if (target.href.slice(0, origin.length) !== origin) {
      return // ignore if not the same origin as the page
    }

    event.preventDefault()
    this.go(target.href.slice(origin.length))
  },
